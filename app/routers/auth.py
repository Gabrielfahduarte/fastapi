...
def login(user_crendetials: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(database.get_db)):
    ...
    if not utils.verify(user_crendetials.password, user.password):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Invalid Crendetials")
